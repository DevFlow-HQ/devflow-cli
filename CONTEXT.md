...
- **Control transport** — how DevFlow launches and steers the provider process. Current contract values: `pty` (via `node-pty`) and `api`.
- **Data plane / event source** — how normalized provider events reach DevFlow. Current contract values: `hooks`, `jsonl`, `logs`, `api`, and `pty` (fallback). Control transport and event source are intentionally orthogonal.
...
- **Hook content** — Codex and Claude hook modes currently provide turn-final assistant content through normalized `turn-completed.assistantMessage` events. Gemini and OpenCode remain PTY fallback providers until their own structured sources are implemented and selected.
- **Fallback tier** — DevFlow models event-source tiers selected before a managed session starts: **hooks → JSONL tailing → PTY synthesis**. Source selection is whole-session and explicit; DevFlow does not mix hooks and JSONL within one session and does not automatically relaunch a failed hook session as JSONL. Today Codex defaults to hooks and can select JSONL internally, Claude can explicitly select hooks or JSONL, and Gemini/OpenCode use PTY fallback.
...
- **PTY fallback events** — normalized events synthesized from PTY terminal output by `runPtyManagedSession`. Used only when no structured event source is selected for the provider (Gemini, OpenCode, and Claude PTY fallback today). For Codex and selected Claude hook/JSONL sessions, PTY is **non-observational**: it serves only as a control transport (spawn, stdin forwarding, terminal mirroring, Ctrl+C, resize) and does not drive completion detection.
